...
| 验收项 | 结果 | 证据 |
|---|---|---|
| **AC-1 对话闭环**：新建会话→发消息→流式事件 | ✅ | `session.create` 返回真实 dsh 会话；`session.send` 返回 messageId；SSE 流出 `message.delta`/`message.user`/`session.started` 事件并进入 store |
...
| **AC-4 断线重连**（验收 4 核心） | ✅ | kill sidecar（SIGTERM）→ bus 快照落盘 → 重启恢复 ring → client 从 lastSeq 增量回放，`before=22, after=24`（旧消息不丢 + 新消息到达，状态一致） |
...
- 本机无 LLM API key，`demo-agent` 的模型调用会失败——**验收验证的是 CKP 协议栈与事件通路**（会话/事件桥接/审批/重连/取消），不是模型回复质量。有 key 的环境只需给 profile 配 provider 即可补测真实流式回复（T-020 验收 1 的「token 平滑」项）。
